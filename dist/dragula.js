...
function dragula (initialContainers, options) {
  ...
  function start (context, event) {
    ...
    drake.dragging = true;
    drake.emit('drag', _item, _source, event);
  }
  ...
  function drop (item, target, event) {
    ...
    if (isInitialPlacement(target)) {
      drake.emit('cancel', item, _source, _source, event);
    } else {
  ...
  function cancel (revert, event) {
    ...
    if (initial || reverts) {
      ...
      drake.emit('drop', item, parent, _source, _currentSibling, event);
    }
    cleanup();
  }
